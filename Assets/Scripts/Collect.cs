...
public partial class Game {
	...
	void SetupCollectibles() {
		...
		SetupCollectible(lifePreserverPrefab, CollectibleType.LifePreserver, false);
	...
	void SetupCollectible(GameObject prefab, CollectibleType type, bool yIsUp) {
		var extents = prefab.GetComponentInChildren<MeshRenderer>().bounds.extents;
		var maxExtent = Mathf.Sqrt(Mathf.Pow(extents.x, 2) + Mathf.Pow(extents.z, 2));
		...
		var interactableObject = instance.AddComponent<VRTK_InteractableObject>();
