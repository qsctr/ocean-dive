...
public class Fish : MonoBehaviour {
	...
	Vector2 Alignment(IEnumerable<Fish> others) => Vector2.ClampMagnitude(Average(
		from other in others where DistanceTo(other) < settings.neighborRadius
		select other.velocity), settings.maxForce);

	Vector2 Cohesion(IEnumerable<Fish> others) => Vector2.ClampMagnitude(Average(
		from other in others where DistanceTo(other) < settings.neighborRadius
